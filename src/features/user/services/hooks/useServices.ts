...
export type ServiceCategory = "Semua" | "STNK" | "BPKB" | "Mutasi" | "SIM" | "Plat Nomor";
...
const mapCategory = (category: string, serviceName: string): ServiceCategory => {
    const catLower = category.toLowerCase();
    const nameLower = serviceName.toLowerCase();
    if (catLower.includes("mutasi") || nameLower.includes("mutasi")) return "Mutasi";
            ...
            if (res?.data) {
                const mapped = res.data.map((item) => {
                    ...
                    return {
                        ...
                        category: mapCategory(item.category, item.service_name),
                        ...
                        options: [
                            ...
                            },
                            {
                                ...
                                extraPrice: Math.max((item.express_fee || 0) - item.service_fee, 0),
                            }
                        ]
                    };
                });
